...
%======================= PARAMETERS =======================%
comp = {'RR', 'RZ', 'ZR'}; %'ZZ'; %'RR'; %'TT';
% coperiod = [5 10]; % Periods to filter between
coperiod = [14 30]; % Periods to filter between
amp = 8e0;
windir = 'window3hr_ellip';
windir_for_SNR = 'window3hr_ellip'; % Data to use for calculating SNR threshold (for plotting purposes)
trace_space = 0; % km
snr_thresh = 2.5;
...
%%
for j = 1:length(comp)
    ccf_path = [ccf_stack_path,'ccf',comp{j},'/',];
    ccf_path_SNR = [parameters.ccfpath,windir_for_SNR,'/','fullStack/','ccf',comp{j},'/',];
    npairall = 0;
    ...
    for ista1=1:nsta % loop over all stations
        ...
        for ista2 = 1: nsta % loop over station pairs
            ...
            sta2dir=[ccf_path,sta2]; % dir to have all cross terms about this central station
            sta2dir_SNR = [ccf_path_SNR,sta2];
            ...
            if ~exist(filename,'file') % check that ccf file exists
                filename = sprintf('%s/%s_%s_f.mat',sta2dir,sta2,sta1);
                filename_SNR = sprintf('%s/%s_%s_f.mat',sta2dir_SNR,sta2,sta1);
                if ~exist(filename, 'file') % check that ccf file exists
    ...
    %             text(0,stapairdist(istapair),dumsta2{istapair})
            ...
            set(gca,'fontsize',15);
        %   pause;
            print(f101,'-dpdf',[figpath,'ccf',comp{j},'_',sta1,'.pdf']); % Save figure
        end
    end % ista1
    ...
    save2pdf([figpath,'all_ccf',comp{j},'_binned_cmap.pdf'],103,500);
    ...
    save2pdf([figpath,'SNR_',comp{j},'_tukeyfilt_',num2str(min_grv),'_',num2str(max_grv),'.pdf'],101,1000);
    %%
    % print(f102,'-dpdf',[figpath,'all_ccf',comp,'_tukeyfilt.pdf']); % Save figure
    if isplotwin
        figname = [figpath,'all_ccf',comp{j},'_tukeyfilt_win',num2str(min_grv),'_',num2str(max_grv),'_TEI19.pdf'];
    else
        figname = [figpath,'all_ccf',comp{j},'_tukeyfilt_TEI19.pdf'];
    end
