...
for ista=1:nsta % loop over all stations
    ...
    for ii = 1:length(files)
        ...
        hv_1_stack = hv.sta1.RZ_ZZ_stack(:,1); % added (:,1) to choose branch 1 NJ
        hv_2_stack = hv.sta1.RR_ZR_stack(:,1); % added (:,1) to choose branch 1 NJ
        hv_av_stack = 0.5 * (hv_1_stack + hv_2_stack);
        hv_RZdiff_stack = 1 - hv_1_stack./hv_2_stack;
        tg_RR_difffromZZ_stack = abs(hv.stapairsinfo.r./hv.grv.RR_stack(:,1) - hv.stapairsinfo.r./hv.grv.ZZ_stack(:,1)); % added (:,1) to choose branch 1 NJ
        tg_RZ_difffromZZ_stack = abs(hv.stapairsinfo.r./hv.grv.RZ_stack(:,1) - hv.stapairsinfo.r./hv.grv.ZZ_stack(:,1)); % added (:,1) to choose branch 1 NJ
        tg_ZR_difffromZZ_stack = abs(hv.stapairsinfo.r./hv.grv.ZR_stack(:,1) - hv.stapairsinfo.r./hv.grv.ZZ_stack(:,1)); % added (:,1) to choose branch 1 NJ
        ...
        wl_vals = [wl_vals; (hv.stapairsinfo.r./(hv.grv.ZZ_stack(:,1).*periods(:)))']; % Change hv.grv.ZZ_stack(:) to hv.grv.ZZ_stack(:,1) so look at 1 branch instead of 2. Add ' to secons variable NJ
%         snr_vals = [snr_vals; 0.25*(hv.snr.ZZ_stack + hv.snr.RR_stack + hv.snr.ZR_stack + hv.snr.RZ_stack)]; % average SNR
    ...
    for ii = 1:length(files)
        ...
        sta1 = hv.stapairsinfo.stanames{1};
        sta2 = hv.stapairsinfo.stanames{2};
        hv_1_stack = hv.sta2.ZR_ZZ_stack (:,1); % added (:,1) to choose branch 1 NJ
        hv_2_stack = hv.sta2.RR_RZ_stack (:,1); % added (:,1) to choose branch 1 NJ
        hv_av_stack = 0.5 * (hv_1_stack + hv_2_stack);
        hv_RZdiff_stack = 1 - hv_1_stack./hv_2_stack;
        tg_RR_difffromZZ_stack = abs(hv.stapairsinfo.r./hv.grv.RR_stack(:,1) - hv.stapairsinfo.r./hv.grv.ZZ_stack(:,1)); % added (:,1) to choose branch 1 NJ
        tg_RZ_difffromZZ_stack = abs(hv.stapairsinfo.r./hv.grv.RZ_stack(:,1) - hv.stapairsinfo.r./hv.grv.ZZ_stack(:,1)); % added (:,1) to choose branch 1 NJ
