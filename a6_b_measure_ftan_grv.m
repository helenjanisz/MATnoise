...
for i=1:length(comp)
    ...
    ftan_out_path = ['./ftan/',windir,'/fullStack/ftan',comp{i},'/',num2str(1/frange_fit(2)),'_',num2str(1/frange_fit(1)),'s_br',num2str(opts.nBranches),'/'];
    if ~exist(ftan_out_path)
        mkdir(ftan_out_path)
    end

    % figure output path
    ftan_fig_path = ['./figs/',windir,'/fullStack/ftan/',comp{i},'_',num2str(1/frange_fit(2)),'_',num2str(1/frange_fit(1)),'s_br',num2str(opts.nBranches),'/'];
    if ~exist(ftan_fig_path)
        mkdir(ftan_fig_path);
    end

    ccf_path = [ccf_stack_path,'ccf',comp{i},'/',];
    npairall = 0;
